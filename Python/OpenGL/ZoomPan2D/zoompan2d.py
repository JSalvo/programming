...
class MouseTrigger(QObject):
	
	def __init__(self):
		QObject.__init__(self)
		
		
	def leftMouseDownPressed(self, x, y):
		print x, " ", y
		self.left_mouse_down.emit(x, y)



class CustomGLWidget(QGLWidget):
	left_mouse_down = pyqtSignal(int, int)
	right_mouse_down = pyqtSignal(int, int)
	middle_mouse_down = pyqtSignal(int, int)
	
	left_mouse_up = pyqtSignal(int, int)
	right_mouse_up = pyqtSignal(int, int)
	middle_mouse_up = pyqtSignal(int, int)	
	...
	def mousePressEvent(self, event):		
		if event.button() == QtCore.Qt.LeftButton:			
			self.left_mouse_down.emit(event.x(), event.y())
		elif event.button() == QtCore.Qt.RightButton:
			self.right_mouse_down.emit(event.x(), event.y())
		elif event.button() == QtCore.Qt.MiddleButton:
			self.middle_mouse_down.emit(event.x(), event.y())
		
	def mouseReleaseEvent(self, event):
		if event.button() == QtCore.Qt.LeftButton:			
			self.left_mouse_up.emit(event.x(), event.y())
		elif event.button() == QtCore.Qt.RightButton:
			self.right_mouse_up.emit(event.x(), event.y())
		elif event.button() == QtCore.Qt.MiddleButton:
			self.middle_mouse_up.emit(event.x(), event.y())
...
class Text:
	...
	def __init__(self, text=[], x=0, y=0, s=1):
		self.__text = text
		self.__x = x
		self.__y = y
		self.__s = s
...
class Attribute:
	def __init__(self, name, x=0, y=0):
		self.__label = Text(name, 20, 0, 0.1)
		self.__x = x
		self.__y = y
